...
FontApiParserTest.prototype.testParsedValuesAreCoherent = function() {
  ...
  assertEquals('font-style: italic;font-weight: bold', droidSerif[0]);
  var yanoneKaffeesatz = variations['Yanone Kaffeesatz'];

  assertNotNull(yanoneKaffeesatz);
  assertEquals(4, yanoneKaffeesatz.length);
  assertEquals('font-weight: 200', yanoneKaffeesatz[0]);
  assertEquals('font-weight: 300', yanoneKaffeesatz[1]);
  assertEquals('font-weight: 400', yanoneKaffeesatz[2]);
  assertEquals('font-weight: 700', yanoneKaffeesatz[3]);
  var cantarell = variations['Cantarell'];

  assertNotNull(cantarell);
  assertEquals(2, cantarell.length);
  assertEquals('font-style: italic', cantarell[0]);
  assertEquals('font-weight: bold', cantarell[1]);
};
...
FontApiParserTest.prototype.testMixOfNumericWeightAndStyle = function() {
  ...
  assertEquals('font-weight: bold', nobile[1]);
  assertEquals('font-style: italic;font-weight: 200', nobile[2]);
  assertEquals('font-style: normal;font-weight: normal', nobile[3]);
};

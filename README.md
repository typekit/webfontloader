...
Note: The earlier way of linking to the latest `1.x` version of the Web Font Loader by using `https://ajax.googleapis.com/ajax/libs/webfont/1/webfont.js` as the `script` source is **no longer supported**. It is stuck to an old version and will not be updated anymore.
